...
class AcceptanceCriteria:
    ...
    correct_patterns: list[CodePattern] = field(default_factory=list)
    incorrect_patterns: list[CodePattern] = field(default_factory=list)
    _language: str = field(default="python", repr=False)
    ...
    def language(self, value: str) -> None:
        """Allow setting language for backward compatibility."""
    ...
    def detect_language(self) -> str:
        ...
        all_patterns = self.correct_patterns + self.incorrect_patterns
        if not all_patterns:
            return "python"
        ...
        if lang_counts:
            detected = max(lang_counts.items(), key=lambda x: x[1])[0]
...
class AcceptanceCriteriaLoader:
    ...
    def load(self, skill_name: str) -> AcceptanceCriteria:
        ...
        if not criteria_path.exists():
            raise FileNotFoundError(f"Acceptance criteria not found: {criteria_path}")

        content = criteria_path.read_text(encoding="utf-8")
        return self._parse_criteria(skill_name, criteria_path, content)

    def _parse_criteria(self, skill_name: str, path: Path, content: str) -> AcceptanceCriteria:
    ...
    def _extract_code_patterns(self, content: str) -> Iterator[CodePattern]:
        ...
        for section in sections:
            ...
            for lang, code in code_blocks:
                ...
                is_correct = self._determine_correctness(code, section_content, is_correct_section)
    ...
    def _determine_correctness(self, code: str, context: str, section_default: bool | None) -> bool:
    ...
    def _extract_rules(self, content: str) -> Iterator[ValidationRule]:
        ...
        for section in sections:
            ...
            if any(
                skip in title.lower() for skip in ["overview", "introduction", "quick reference"]
            ):
                continue
