...
class SkillEvaluationRunner:
    ...
    def run(self, skill_name: str, scenario_filter: str | None = None) -> EvaluationSummary:
        ...
        if scenario_filter:
            scenarios = [
                s
                for s in scenarios
                if scenario_filter.lower() in s.name.lower() or scenario_filter.lower() in s.tags
            ]
        ...
        for scenario in scenarios:
            ...
            if scenario.mock_response and self._use_mock:
                self.copilot_client.add_mock_response(scenario.name, scenario.mock_response)
            ...
            # Evaluate
            eval_result = evaluator.evaluate(gen_result.code, scenario=scenario.name)

            # Add scenario-specific checks
            self._check_scenario_patterns(eval_result, scenario, gen_result.code)
    ...
    def _check_scenario_patterns(
        self, result: EvaluationResult, scenario: TestScenario, code: str
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        "skill", nargs="?", help="Skill name to evaluate (e.g., azure-ai-agents-py)"
    )
    parser.add_argument(
        "--list", action="store_true", help="List available skills with test scenarios"
    )
    parser.add_argument("--filter", help="Filter scenarios by name or tag")
    parser.add_argument(
        "--mock", action="store_true", help="Use mock responses instead of Copilot SDK"
    )
    ...
    if args.output == "json":
        ...
        lines = [
            ...
            f"Pass Rate: {summary.passed / summary.total_scenarios * 100:.1f}%"
